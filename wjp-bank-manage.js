...
(function () {
  ...
  // --------------------------------------------------------------------------
  // Bank Health placement — inject into the Bank Health Check modal
  // The existing modal is `#bank-health-overlay` with body `#bh-body`.
  // We prepend our "Linked Bank Accounts" card as the first child of #bh-body
  // each time the modal opens, then refresh its summary.
  // --------------------------------------------------------------------------
  var CARD_ID = 'wjp-bank-manage-bh-card';

  function findBankHealthHost() {
    var overlay = document.getElementById('bank-health-overlay');
    if (!overlay) return null;
    var s = getComputedStyle(overlay);
    var visible = overlay.offsetParent !== null && s.display !== 'none' && s.visibility !== 'hidden';
    if (!visible) return null;
    return document.getElementById('bh-body') || overlay;
  }

  async function mountBankHealthCard(forceRefresh) {
    var host = findBankHealthHost();
    if (!host) {
      // Modal closed — drop the card so it doesn't ghost into the next opening
